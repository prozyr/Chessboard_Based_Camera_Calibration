...
class CameraApplication:
    ...
    def __init__(self, chessboard_size, camera_number, heigth, width):
        ...
        #UI
        self.window = tk.Tk()
        # self.window.geometry('200x150') tkinter window size
        self.window.wm_title("Camera calibration")
        self.window.config(background="#263A52")

        self.original_imageFrame = tk.Frame(self.window, width=self.width, height=self.heigth)
        self.original_imageFrame.grid(row=0, column=0, padx=2, pady=2)

        self.chessBoardDetection_imageFrame = tk.Frame(self.window, width=self.width, height=self.heigth)
        self.chessBoardDetection_imageFrame.grid(row=0, column=1, padx=2, pady=2)

        self.undistorted_imageFrame = tk.Frame(self.window, width=self.width, height=self.heigth)
        ...
        ##Bottom frame
        self.bottom_frame = tk.Frame(self.window, width=self.width, height=self.heigth/2)
    ...
    def show_frame_wrapper(self):
        ...
        frame = cv2.resize(frame, (self.heigth,self.width))
